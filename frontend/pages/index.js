...
function FeedItem({ item }) {
  return (
    <div style={{
      ...
      <span style={{ color: item.color, fontSize: 11, fontWeight: 800 }}>
        {item.ownerName}
      </span>
      <span style={{ color: 'var(--text-soft)', fontSize: 11, fontWeight: 600 }}>
        claimed #{item.cellId} 🎨
      </span>
    </div>
  );
...
function ZoomControls({ zoom, onZoom }) {
  return (
    <div style={{
      ...
      position: 'fixed', bottom: 24, right: 272,
      zIndex: 100,
    }}>
...
export default function Home() {
  ...
  const { connected: socketConnected, claimBlock } = useSocket({
    user,
  ...
  const handleZoom = useCallback((delta) => {
    setZoom((z) => {
      if (delta === 0) return 18;
      return Math.max(6, Math.min(32, z + delta * 3));
    });
  }, []);

  useEffect(() => {
    const onWheel = (e) => {
      if (e.ctrlKey) { e.preventDefault(); handleZoom(e.deltaY < 0 ? 1 : -1); }
    };
    ...
    <>
      ...
      <div style={{
        ...
        <header style={{
          ...
          boxShadow: '0 2px 20px rgba(255,110,180,0.08)',
        }}>
          {/* Logo */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 16, boxShadow: '0 4px 12px rgba(255,110,180,0.35)',
            }}>🌸</div>
            <h1 style={{
              fontFamily: 'Fredoka One, cursive',
              fontSize: 22,
          ...
          <div style={{
            fontSize: 11, color: 'var(--text-muted)',
            display: 'flex', gap: 14, fontWeight: 700,
          }}>
          ...
          <div style={{
            ...
            padding: '5px 12px',
            background: socketConnected ? '#E8FFF4' : '#FFF0F0',
            border: `1.5px solid ${socketConnected ? '#34D39966' : '#FF6B6B66'}`,
            borderRadius: 99,
            ...
            <div style={{
              ...
              background: socketConnected ? '#34D399' : '#FF6B6B',
              boxShadow: socketConnected ? '0 0 8px #34D399' : 'none',
          ...
          <div style={{ flex: 1, position: 'relative', overflow: 'hidden', borderRadius: 16 }}>
            {loading ? (
              ...
              <Grid
                ...
                userId={user?.id} onClaim={handleClaim} zoom={zoom}
              />
            )}
            ...
            {feed.length > 0 && (
              <div style={{
                ...
                <div style={{
                  ...
                  fontWeight: 800, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1,
                }}>
                  ✨ activity
                </div>
